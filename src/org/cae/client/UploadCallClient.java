...
public class UploadCallClient {
	...
	private UploadResult upload(String path) {
		...
		try {
			...
			ds.writeBytes("Content-Disposition: form-data;name=\"callVersion\""
					+ end);
			ds.writeBytes(end);
			...
			if (httpURLConnection.getResponseCode() == HttpURLConnection.HTTP_OK) {
				...
				JSONObject result = JSONObject.fromObject(resultBuffer
						.toString());
				if (result.getBoolean("successed")) {
					theResult = new UploadResult();
				} else {
					theResult = new UploadResult(
							result.getBoolean("successed"),
							(List<String>) result.get("failList"),
							result.getString("errInfo"));
				}
			}
